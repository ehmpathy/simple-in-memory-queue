...
import { QueueOrder } from '@src/domain.objects/constants';
import { createQueue } from '@src/domain.operations/queue/createQueue';
...
export const createQueueWithResilientRemoteConsumer = <T>({
  ...
  const consume = withSimpleCache(
    ...
    {
      cache: createCache({
        expiration: { seconds: 0 }, // expire the promise as soon as it resolves -> dont allow duplicate invocations
      }),
    },
  );
